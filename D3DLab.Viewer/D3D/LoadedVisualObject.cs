    ...
    class LoadedVisualObject : MultiVisualObject {
        ...
        static void _Create(IContextState context, IEnumerable<IFileGeometry3D> meshes,
            ...
            foreach (var geo in meshes) {
                var color = geo.Colors.Any() ? geo.Colors[0] : V4Colors.Green.XYZ();
                var tag = Create(context, 
                    baseTag.WithPrefix(geo.Name ?? index.ToString()),
                    new GeometryStructures<IFileGeometry3D>(geo), 
                    texture,
                    color,
                    out var box);
                t.Add(tag);
        ...
        static ElementTag Create(IContextState context, ElementTag tag, GeometryStructures gdata, FileInfo texture, Vector3 defColor,
            ...
            if (gdata.TexCoor.Any() && texture != null) {
                ...
                material = MaterialColorComponent.Create(
                    V4Colors.NextColor(random)
                );
        ...
        public AxisAlignedBox GetAllBounds(IContextState context) {
            var fullBox = AxisAlignedBox.Zero;
            var manager = context.GetEntityManager();
            foreach (var t in Tags) {
                ...
                var box = en.GetComponent<GeometryBoundsComponent>()
                    .Bounds
                    .Transform(tr.MatrixWorld);
               
                fullBox = fullBox.IsZero ? box : fullBox.Merge(box);
            }
            return fullBox;
        }
